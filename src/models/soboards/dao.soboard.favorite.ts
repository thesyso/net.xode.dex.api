...
const etList = async (conn: any, params: any) => {
  ...
  if (srUsed) {
    ...
    vQuery = vQuery + ` WHERE sf.favorite_id IS NOT NULL`;
  }
  ...
  if (sr != 0 && srTxt.length > 0) {
    switch (true) {
      case sr == 1:
        vParams.push(srTxt);
        vQuery = vQuery + ` AND sf.soboard_id = ?`;
        break;
      case sr == 2:
        vParams.push(srTxt);
        vQuery = vQuery + ` AND sf.wallet_id = ?`;
        break;
    }
  }
  ...
  vQuery = vQuery + ` ORDER BY sf.favorite_id DESC `;
