...
const etList = async (conn: any, params: any) => {
  ...
  if (srUsed) {
    vParams.push(srUsed);
    vQuery = vQuery + ` WHERE b.is_use = ?`;
  } else {
    vQuery = vQuery + ` WHERE b.coin_code IS NOT NULL`;
  }
  ...
  if (sr != 0 && srTxt.length > 0) {
    switch (true) {
      case sr == 1:
        vParams.push(srTxt);
        vQuery = vQuery + ` AND b.coin_code LIKE ?`;
        break;
      case sr == 2:
        vParams.push(srTxt);
        vQuery = vQuery + ` AND b.coin_name LIKE ?`;
        break;
    }
  }
...
const etSave = async (conn: any, params: any) => {
  ...
  var vQuery = `
    ...
    `;
  vParams.push(params.coin_code, params.coin_name);
  return await conn.query(vQuery, vParams);
};
// 수정
...
const etRemove = async (conn: any, ucode: string) => {
  ...
  vParams.push(ucode);
  var vQuery = `
        UPDATE base_coin SET is_use = CASE WHEN is_use = 1 THEN 0 ELSE 1 END
        WHERE coin_code = ?
  `;
   return await conn.query(vQuery, vParams);
};
