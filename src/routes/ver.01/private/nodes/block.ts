...
import blockController from "../../../../controllers/nodes/block.js";
...
router.get('/', async (req, res) => {
  ...
  const { success, data, message } = await blockController.acList(sParams);
  res.status(200).json({ success, data, message });
});

router.delete('/:id', async (req, res) => {
  const id = req.params?.id ? req.params.id.toString() : "";
  const { success, data, message } = await blockController.acRemove(id);
  res.status(200).json({ success, data, message });
});
