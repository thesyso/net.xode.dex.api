...
import { verify, verifyWallet } from "../libs/modules/auth.token";
...
export const mwWalletAuthJWT = async (req: Request, res: Response, next: NextFunction) => {
    if (req.headers.authorization) {
        const accessToken = req.headers.authorization.split('Bearer ')[1]; // header에서 access token을 가져옵니다.
        const result = verifyWallet(accessToken); // token을 검증합니다.
