...
import { refreshVerifyWallet, verify, verifyWallet } from "../libs/modules/auth.token";
...
export const mwMasterAuthJWT = async (
  req: Request,
  res: Response,
  next: NextFunction,
) => {
...
export function mwAuth(req: any, res: any, next: any) {
  ...
  if (!token) {
    return res
      .status(401)
      .json({ success: false, message: "유효한 인증 정보가 없습니다." });
  }
  // 토큰 검증 로직 (예: JWT 검증)
...
// const ischeckWalletAuth = (bearerToken: string, mode: string = "access") => {
...
//     if(mode === "access") {
//       resVerify = verifyWallet(bearerToken);
//     } else if(mode === "refresh") {
//       resVerify = refreshVerifyWallet(bearerToken);
//     }
